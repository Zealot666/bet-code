    ...
    try:
        ...
        for tr in list(doc('tr').items())[1:]:
            ...
            if len(tds) == 15:
                try:
                    print(tr)
                    if tds[10].text().strip() != "" and tds[3].text().strip() != "完" and tds[
                        3].text().strip() != "中":
...
def find_good_matchv2(idList):
    ...
    try:
        ...
        for i in res.to_dict(orient='records'):
            ...
            print(history, i["per"], url)
            if i["per"] >= 0.61 \
                    and ((i["masterOdd_Start_Last_3"] >= 0.80 and (0.5 == daxiao_num(i["pankou_Start_Last_3"]) - sum1))) \
                    and i["lastBifengNow"] == i["lastBifeng"] and i["lastTime"] <= 75 \
                    and i["place"] not in history.keys():
                matchList.append(i)
            num += 1
        ...
        if len(matchList) > 0:
            ...
            for i in matchList:
                ...
                print("test bet one match 1.00", i["masterOdd_Start_Last_3"], zhuDui, keDui)
                history[i["place"]] = 1
                if zhuDui != "" and keDui != "":
                    sendList = sendList + zhuDui + "," + keDui + "|"
...
if __name__ == '__main__':
    while True:
        ...
        print("urllist222", urlList)
        find_matches(urlList, "daxiao")
        print("epoch", len(testList))
        end = time.time()
        if len(testList) != 0:
